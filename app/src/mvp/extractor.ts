...
// Minimum signal hits required per block type.
const MIN_HITS: Record<BlockType, number> = {
  Feature:  2,
  Decision: 2,
  Risk:     2,
  Todo:     2,
...
// Max blocks of the same type extracted from a single source.
const MAX_PER_SOURCE_TYPE = 3;

// Hard cap on total blocks returned (kept by descending confidence).
const MAX_TOTAL_BLOCKS = 45;

export function extractBlocks(chunks: MemoryChunk[]): MemoryBlock[] {
  const blocks: MemoryBlock[] = [];
  // Track per-(sourceName, type) count to enforce cap.
  const perSourceType = new Map<string, number>();
  let i = 0;
  ...
  for (const chunk of chunks) {
    ...
    for (const rule of RULES) {
      ...
      if (hits < MIN_HITS[rule.type]) continue;

      // Enforce per-source-type cap.
      const capKey = `${chunk.sourceName}::${rule.type}`;
      const count = perSourceType.get(capKey) ?? 0;
      if (count >= MAX_PER_SOURCE_TYPE) continue;
      perSourceType.set(capKey, count + 1);
  ...
  // Keep top MAX_TOTAL_BLOCKS by confidence.
  return deduped
    .sort((a, b) => b.confidence - a.confidence)
    .slice(0, MAX_TOTAL_BLOCKS);
}
...
/**
 * Normalize title for dedup: lowercase, strip punctuation, keep first 50 chars.
 * Catches near-duplicate blocks extracted from slightly different chunks.
 */
